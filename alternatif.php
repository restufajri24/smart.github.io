...
?>
	...
	if(isset($_POST['simpan'])){
		$nama = $_POST['nama'];
		$stmt2 = $db->prepare("insert into smart_alternatif(nama_alternatif) values('$nama')");
	...
	<form method="post">
		...
		<select name="nama">
		    	...
		    	<?php
				$stmt3 = $db->prepare("select * from smart_mahasiswa");
				$stmt3->execute();
				while($row3 = $stmt3->fetch()){
				?>
		    	<option value="<?php echo $row3['nama_mahasiswa'] ?>"><?php echo $row3['nama_mahasiswa'] ?></option>
		    	<?php
		    	}
		    	?>
		</select>
...
<table class="table striped hovered cell-hovered border bordered dataTable" data-role="datatable" data-searching="true">
	...
	<tbody>
		...
		<tr>
			...
			<td>
			<?php
				$stmt2 = $db->prepare("select * from smart_mahasiswa where nama_mahasiswa='".$row['nama_alternatif']."'");
				$stmt2->execute();
